...
OS1_CNV_calling<-read_excel("/Users/catincaapostol/Desktop/Master's/project/wgs os1/OS1_ploidy_cnv.xlsx")
...
#Determine the mean cn for the genes with more than one cn value, if it is a deciman CN, round up
...
#merge with protein coding for ordering 
OS1_final_cn_order<-merge(OS1_final_cn, Protein_type_coding, by = "gene")

#There are some genes that are not in the original protein coding list that I am using to do the merging. I am going to ignore there for now:
setdiff(OS1_final_cn$gene,OS1_final_cn_order$gene)
#"LINC01556" "LY6G6E"    "SNHG32"    "C4B_2"     "OR8U8"     "OR9G9"     "GOLGA8DP"  "CCL3L1"    "KIR3DX1"  "KIR2DS5"   "KIR2DS3"   "KIR2DS2"   "KIR2DL2"   "KIR2DS1"   "KIR3DS1"
...
#obtain a final df with gene name, chromosome, cn and order
...
####
#To make copy number plots you I need the chromosome strat and end positions
...
#Make copy number plots 
##OHS
OS1_copy_number_outrem<-OS1_copy_number
ggplot(OS1_copy_number_outrem, aes(x = order, y = cn)) +
  geom_point(size = 0.1, color = "lightgrey", position = position_jitter(width = 0.5, height = 0.5)) +  # Customize point size and color
  theme_minimal() +                       # Use a minimal theme
  labs(x = "Order", y = "CN", title = "Dot Plot of Order vs. CN") +  # Labels
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

average_data_OS1 <- OS1_copy_number_outrem %>%
  group_by(order) %>%
  summarise(average_cn = mean(cn), .groups = 'drop')


OS1_copy_number_outrem <- OS1_copy_number_outrem %>%
...
OS1_cn_wholegenome<-ggplot(OS1_copy_number_outrem, aes(x = order, y = cn)) +
  geom_point(aes(color = color), size = 3, position = position_jitter(width = 0.5, height = 0.5),alpha = 1) +
  geom_segment(data = average_data_OS1, aes(x = order, xend = order, y = average_cn, yend = average_cn), color = "black", size = 0.5) +
  geom_point(data = average_data_OS1, aes(x = order, y = average_cn), color = "black", size = 0.001, shape = 0.01, fill = "black") +  # Increased size for better visibility
  geom_vline(data = positions_OS1, aes(xintercept = largest_order_OS1), linetype = "dashed", color = "grey", size = 0.5) +  # Customize color and linetype
  geom_text(data = positions_OS1, aes(x = mid, y = -0.7, label = chromosome), 
            vjust = 1, hjust = 0.5, color = "black", size = 7) + 
  theme_classic() + 
  labs(x = "Chromosome", y = " Gene copy number") +  
  theme(axis.text.x = element_text(size = 18)) +
  theme(axis.text.y = element_text(size = 20))+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1)) +
  theme(axis.title.x = element_text(size = 20))+
  theme(axis.title.y = element_text(size = 20))+
  scale_y_continuous(breaks = 0:25, limits = c(-1, 25))+
  scale_x_discrete(expand = c(0, 0))+ 
  scale_color_identity()

print(OS1_cn_wholegenome)
ggsave(
  "/Users/catincaapostol/Desktop/Master's/project/OS1_copy_number_plot.png", 
  plot = OS1_cn_wholegenome,
  width = 17, 
  height = 6, 
  units = "in", 
  dpi = 300 
)
#ggsave("/Users/catincaapostol/Desktop/Master's/wgs os1 analysis plots/OS1_copynumber_wholegenome_dotplot.jpg", plot = OS1_cn_wholegenome, width = 15, height = 10, dpi = 300)



cn_counts_OS1 <- OS1_copy_number_outrem %>%
  group_by(cn) %>%           
  summarise(count = n()) %>% 
  arrange(cn) 
total_count_OS1 <- sum(cn_counts_OS1$count)
cn_counts_OS1 <- cn_counts_OS1 %>%
  mutate(frequency_OS1 = (count / total_count_OS1) *100)

cn_counts_OS1$cn <- as.numeric(cn_counts_OS1$cn)

# Create the plot
bell_curve_plot_OS1 <- ggplot(cn_counts_OS1, aes(x = cn, y = frequency_OS1)) + 
  geom_bar(stat = "identity", fill = "black", width = 0.8, alpha = 0.7) +  # Add bars with light blue fill
  labs(x = "Copy Number", y = "Frequency count (%)", title = "Bell Curve-like Distribution of Copy Number OS1") + 
  theme_classic() +  # Clean theme
  scale_x_continuous(breaks = seq(min(cn_counts_OS1$cn), 25, by = 1)) +  # Set x-axis breaks every 2 units
  scale_y_continuous(breaks = seq(0, 70, by = 10), limits = c(0, 70),expand = c(0, 0)) +  # Set y-axis breaks every 10, with limits from 0 to 50
  theme(
    axis.text.x = element_text(size = 6),   # Customize x-axis text size
    axis.text.y = element_text(size = 6), 
    axis.title.x = element_text(size = 7),  # Decrease x-axis label size
    axis.title.y = element_text(size = 7),  # Customize y-axis text size
    plot.title = element_text(size = 7.5, hjust = 0.5)   # Center and resize the title
  )
 print(bell_curve_plot_OS1)
# Save the plot
ggsave("/Users/catincaapostol/Desktop/Master's/wgs os1 analysis plots/bell_curve_plot_OS1.jpg", 
       plot = bell_curve_plot_OS1, 
       width = 3, 
       height = 3, 
       dpi = 300)
...
######
##Zoom in on chr17
Mart<-read.csv("/Users/catincaapostol/Desktop/Master's/hgnc_complete_set_subset_order_CHROMOSOME_kb.csv")
...
cyto <- read.table(
  "/Users/catincaapostol/Desktop/Master's/wgs os1/cytoBand.txt",
  sep = "\t",
  header = FALSE,
...
# --- BEGIN: drop-in block to create gene_mid and Karyotype.band ---
# (Paste this immediately after you have created order_17 and order_17copy)

# list of candidate column names for gene start / end (common variants)
possible_starts <- c(
...
# build numeric gene_mid (use midpoint if end exists, otherwise use start)
...
# ensure cyto17 exists and has numeric starts/ends
if (!exists("cyto17")) stop("cyto17 not found. Create cyto17 <- cyto[cyto$chrom == 'chr17', ] before this block.")
cyto17$chromStart <- as.numeric(cyto17$chromStart)
cyto17$chromEnd   <- as.numeric(cyto17$chromEnd)

# assign each gene to a cyto band (using bp intervals)
...
# report small diagnostics to console
...
# ===============================
# CYTOBAND INTEGRATION (UCSC)
# ===============================

# cyto17 must already exist:
# columns: chrom, chromStart, chromEnd, band, stain

# assign each gene to a cytoband based on order
cyto17$order_band_start <- seq_len(nrow(cyto17))

# reuse gene order as proxy x-axis for band mapping
# assign cytoband by genomic position (NOT by order)
...
# ===============================
# BAND COORDINATES (UNCHANGED LOGIC)
# ===============================

# add space at centromere
...
positions_kb_OS1$largest_order[
  positions_kb_OS1$kb == "p11.2" &
    positions_kb_OS1$largest_order > 323
] <- 322
...
OS1_cn_17_ploidy <- ggplot() + 
  ...
  geom_text(data = darklables, 
            aes(x = mid, y = -1.5, label = kb),  # Adjust y position as needed
            color = "black", size = 2.5, vjust = -0.5,fontface = "bold") +
