...
vcf_file <- "/Users/catincaapostol/Desktop/Master's/project/wgs os1/candidateSV.vcf.gz"
...
# Parse chr:pos from ALT bracket strings robustly
...
# nearest gene within 10kb if none overlapping
...
canonical_chrs <- c(paste0("chr", 1:22), "chrX", "chrY")
...
bp_max <- 21883318 
sel_dt <- plot_all[
  (plot_all$chr1 == focal & plot_all$pos1 >= bp_min & plot_all$pos1 <= bp_max) |
    (plot_all$chr2 == focal & plot_all$pos2 >= bp_min & plot_all$pos2 <= bp_max) |
    (plot_all$chr1 == "17"   & plot_all$pos1 >= bp_min & plot_all$pos1 <= bp_max) |
    (plot_all$chr2 == "17"   & plot_all$pos2 >= bp_min & plot_all$pos2 <= bp_max), 
]
...
partner_chr <- sel_dt$chr2
partners_unique <- sort(unique(partner_chr))
focal <- "chr17"
partners_unique <- partners_unique[partners_unique != focal]

if(!all(grepl("^chr", region1_all$chr))) region1_all$chr <- paste0("chr", region1_all$chr)
if(!all(grepl("^chr", region2_all$chr))) region2_all$chr <- paste0("chr", region2_all$chr)

nparts <- max(1, length(partners_unique))
my_dark_colors <- c("#006400", "#2F4F4F", "#4B0082", "#2A6F97", "#3B4994")
cols_palette <- colorRampPalette(my_dark_colors)(nparts)
...
canonical_chrs <- c(focal, partners_unique)
draw_circos_page <- function(page_title, region1_local, region2_local, row_colors_hex) {
  used_chrs <- unique(c(region1_local$chr, region2_local$chr))
  ...
  circos.initializeWithIdeogram(species = "hg38", 
                                chromosome.index = used_chrs, 
                                ideogram.height = 0.12)
  
  circos.trackPlotRegion(track.height = 0.05, ylim = c(0,1), bg.border = NA,
                         panel.fun = function(region, value, ...) {
                           circos.text(mean(CELL_META$xlim), 0.5, CELL_META$sector.index, cex = 0.7, niceFacing = TRUE)
                         })
  ...
  if(length(legend_labels) > 0) {
    par(xpd = TRUE)
    legend("topright", legend = legend_labels, fill = legend_cols, border = NA, bty = "n", cex = 0.8, inset = c(0.01, 0.02))
  }
  

  circos.clear()
}
ggplot_style_wrapper <- function(...) {
  plot_args <- list(...)
  structure(list(args = plot_args), class = "gg_circos")
}

print.gg_circos <- function(x, ...) {
  do.call(draw_circos_page, x$args)
  invisible(NULL)
}


p <- ggplot_style_wrapper(
  region1_local = region1_all,
  region2_local = region2_all,
  row_colors_hex = row_partner_colors_hex
)
pdf("/Users/catincaapostol/Desktop/Master's/project/wgs os1/os1_amp_reg_translocations.pdf", width = 7, height = 5)
...
pdf("/Users/catincaapostol/Desktop/Master's/project/wgs os1/os1_all_translocations.pdf", width = 7, height = 5)
message("Writing chr17_translocations.pdf (page1 = incoming->17 colored by source; page2 = outgoing from 17 colored by destination)")

p <- draw_circos_page(
  page_title = paste0("os1 translocations"),
  region1_local = region1_all,
  region2_local = region2_all,
  row_colors_hex = row_partner_colors_hex
)
